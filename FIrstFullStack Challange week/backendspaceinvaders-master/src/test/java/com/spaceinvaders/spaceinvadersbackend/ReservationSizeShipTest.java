...
public class ReservationSizeShipTest {
    @Mock
    ...
    @Test
    public void should_throw_exception_when_spaceship_size_does_not_match_dock_size() {
        dock.setSpaceSize(Size.M);
        spaceship.setSize(Size.L);
        ...
        RuntimeException exception = assertThrows(RuntimeException.class, () -> reservationDAO.createReservation(reservationDTO));

        assertEquals("Spaceship doesn't have the correct space size", exception.getMessage());
    }

    @Test
    public void should_create_reservation_when_spaceship_size_matches_dock_size() {
        when(userFromTokenService.getCurrentUser()).thenReturn(user);
        when(dockRepository.findById(dock.getId())).thenReturn(Optional.of(dock));
        when(spaceshipRepository.findById(spaceship.getShipId())).thenReturn(Optional.of(spaceship));
